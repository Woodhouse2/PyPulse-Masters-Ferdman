...
'''
# Edits to tscrunch, fscrunch & scrunch methods by Ryan L. Woodhouse (2019).

import math # Required for error capture and rounding up. 
import numpy as np
...
class Archive:
    ...
    def load(self,filename,prepare=True,center_pulse=True,baseline_removal=True,weight=True,wcfreq=False):
        ...
        DAT_FREQ = hdulist['SUBINT'].data['DAT_FREQ']                                                       #  Frequncies Loaded
        ...
        self.freq = DAT_FREQ                                                                                  #  Frequncies Loaded as self.freq
    ...
    def scrunch(self,arg='Dp',**kwargs):                       # Necessary? (Repurpose such that scrunch calls other methods rather than vice versa)?
        """average the data cube along different axes"""
        self.record(inspect.currentframe())
        if 'T' in arg:
            self.tscrunch() # Easier for scrunch to call tscrunch rather than vice versa for one specific case.
            '''
        ...
        if 'F' in arg:
            self.fscrunch() # Easier for scrunch to call fscrunch rather than vice versa for one specific case.
            '''
    ...
    def tscrunch(self,nsubint=None,factor=None): # Default is none.
        """average the data cube along the time dimension"""
        # If factor that is greater than 1 is given, given nsubint is ignored (given nsubint is only used to calculate the factor). 
        if (str(factor).isdigit() == False) and factor is not None: #  Floor for when factor is not an integer and isn't nothing.
            factor = math.floor(factor)
        if (factor is not None and factor <= 1) or (nsubint is not None and nsubint >= self.getNsubint()): #  Reordered if statements for efficiency.  
        #  Added in case for when number of subints given is equal to (or erroneously greater than) the number of subints already.
        #  Added in case for when factor is erroneously less than 1.
            return self
        if (str(nsubint).isdigit() == False) and nsubint is not None: #  Added in floor for when nsubint is not an integer and isn't nothing.
            nsubint = math.floor(nsubint)
        # if nsubint == 1 or (factor is None and nsubint is None):        [Invocation the scrunch method doesn't seem to be necessary, hence "if" statement has been changed]
        if (factor is None and nsubint is None) or (nsubint is not None and nsubint <= 1): # or (factor >= self.getNsubint()) {redundant, as "newnsubint" will be 1 due to rounding up}:   # Default case [Added in case when nsubint is erroneously less than one]
            # return self.scrunch('T')                                    
            factor = self.getNsubint()
        if factor is None and nsubint is not None:
            factor = math.ceil(self.getNsubint()/nsubint) # Already loaded math class, more concise way to round up.
        #   factor = self.getNsubint()//nsubint              # nsubint is the number of sub-integrations for the NEW tscrunched data here.
        #   if self.getNsubint()%nsubint != 0:
        #       factor += 1                                  
        self.record(inspect.currentframe())


        nsubint = self.getNsubint() # After this point, nsubint is the number of sub-integrations for the OLD untscrunched data.
        npol = self.getNpol()
        nchan = self.getNchan() # Updates the number of channels if fscrunch has been performed beforehand.
        nbin = self.getNbin()
        
        retval = np.zeros((len(np.r_[0:nsubint:factor]),npol,nchan,nbin))
        newdurations = np.zeros(np.shape(retval)[0])

        #weightsum = np.sum(self.weights) #  Data Division Fix [see below].
        weightretval = np.zeros((len(np.r_[0:nsubint:factor]),nchan)) #  Need similar line for the weighted fscrunched frequencies:
        freqWgtRetval = np.zeros((len(np.r_[0:nsubint:factor]),nchan)) # Seen here
            
        # &&    
        #newnsubint = nsubint//factor # Issue in not including all subints due to rounding down.  
        newnsubint = math.ceil(nsubint/factor) # Rounds up instead of down.  
        
        freq = self.freq # Loads the frequencies in. 
        wgts = self.weights # Loads the weights in. 
        
        # 
        #--------------------------
        # Multiply the weightings with the frequencies:
        freqWgt = freq*wgts
        #--------------------------
        
        
        for i in xrange(newnsubint):
            weightretval[i,:] = np.sum(wgts[i*factor:(i+1)*factor,:],axis=0) # tscrunches the weightings
            freqWgtRetval[i,:] = np.sum(freqWgt[i*factor:(i+1)*factor,:],axis=0) # tscrunches the weighted frequencies
            ...
            for j in xrange(npol):
                for k in xrange(nchan):
                    for l in xrange(nbin):
                        retval[i,j,k,l] = np.sum(self.data[i*factor:(i+1)*factor,j,k,l] * wgts[i*factor:(i+1)*factor,k])
                        with np.errstate(divide='ignore', invalid='ignore'):
                            retval[i,j,k,l] /= weightretval[i,k] # Data Division Fix.
                            if math.isnan(retval[i,j,k,l]) == True:
                                retval[i,j,k,l] = 0
        ...
        # Required for when there is "0/0" in the element wise matrix division. Masks the error and replaces "NaN" result with 0. 
        with np.errstate(divide='ignore', invalid='ignore'):
            self.freq = freqWgtRetval/weightretval  #  Division by the weightings
                
            # Handles the case where a "NaN" is given.    
            for n in range(0,int(newnsubint)):
                for k in range(0,int(nchan)):
                    if math.isnan(self.freq[n,k]) == True:
                        self.freq[n,k] = 0
    ...
    def fscrunch(self,nchan=None,factor=None):
        ...
        # If factor that is greater than 1 is given, given nchan is ignored.
        # Floor for when factor is not an integer and isn't nothing.
        if (str(factor).isdigit() == False) and factor is not None:
            factor = math.floor(factor)
        if (factor is not None and factor <= 1) or (nchan is not None and nchan >= self.getNchan()):
            return self
        # Added in floor for when nchan is not an integer and isn't nothing.
        if (str(nchan).isdigit() == False) and nchan is not None:
            nchan = math.floor(nchan)
        #if nchan == 1 or (factor is None and nchan is None): # See corresponding "tscrunch" comments
        if (factor is None and nchan is None) or (nchan is not None and nchan <= 1): # or (factor >= self.getNchan()): (see tscrunch for redundancy reason)
            #return self.scrunch('F')
            factor = self.getNchan()
        if factor is None and nchan is not None:
            factor = math.ceil(self.getNchan()/nchan)
        #   factor = self.getNchan()//nchan # nchan is the number of frequency channels for the NEW fscrunched data here.
        #   if self.getNchan()%nchan != 0:
        #       factor += 1                                            
        self.record(inspect.currentframe())

        nsubint = self.getNsubint() # Updates the number of sub-integrations if tscrunch has been performed beforehand. [If factor is given, subint need not be given]                             
        npol = self.getNpol()
        nchan = self.getNchan() # After this point, nchan is the number of frequency channels for the OLD unfscrunched data.
        nbin = self.getNbin()
        ...
        #weightsum = np.sum(self.weights) #  Data Division Fix. (See Below)
        weightretval = np.zeros((nsubint,len(np.r_[0:nchan:factor]))) #  Need similar line for the weighted fscrunched frequencies (Set up the scrunched arrays):
        freqWgtRetval = np.zeros((nsubint,len(np.r_[0:nchan:factor]))) # Seen here

        #newnchan = nchan//factor # (Issue, see tscrunch method)
        newnchan = math.ceil(nchan/factor) # (Correction, see corresponding tscrunch code)
        
        freq = self.freq # Loads the weights in. 
        wgts = self.weights # Loads the weights in. 
        '''
        ...
        if np.ndim(self.freq) == 1:
            ...
            freq = self.freq[0] #  Just gets the frequency labels for the first sub-integration(?)
        newfreq = np.zeros(newnchan) #this will only be 1D
        '''
        
        # 
        #--------------------------
        # Multiply the weightings to the frequencies:
        freqWgt = freq*wgts
        #--------------------------
        
        # NEW:
        #  fscrunches the weightings and the weighted frequencies.
        for k in xrange(newnchan):
            weightretval[:,k] = np.sum(wgts[:,k*factor:(k+1)*factor],axis=1) # fscrunches the weightings
            freqWgtRetval[:,k] = np.sum(freqWgt[:,k*factor:(k+1)*factor],axis=1) # fscrunches the weighted frequencies 
        # Old:    
            # newfreq[k] = np.sum(freq[k*factor:(k+1)*factor])/float(factor) #unweighted! #  Just fscrunches 1 sub-integration's "frequency label" and uses that as the new frequency label (Presumably for every sub-integration). 
        
        for i in xrange(nsubint):
            for j in xrange(npol):
                for k in xrange(newnchan):
                    for l in xrange(nbin):
                        retval[i,j,k,l] = np.sum(self.data[i,j,k*factor:(k+1)*factor,l] * wgts[i,k*factor:(k+1)*factor])
                        with np.errstate(divide='ignore', invalid='ignore'):
                            retval[i,j,k,l] /= weightretval[i,k] # Data Division Fix.
                            if math.isnan(retval[i,j,k,l]) == True:
                                retval[i,j,k,l] = 0
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            # Handles the case where a "NaN" is given.    
            for n in range(0,int(nsubint)):
                for k in range(0,int(newnchan)):
                    if math.isnan(self.freq[n,k]) == True:
                        self.freq[n,k] = 0
